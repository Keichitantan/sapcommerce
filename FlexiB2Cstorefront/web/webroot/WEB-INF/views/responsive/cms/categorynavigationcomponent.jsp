		...
		<div class="navigation__overflow">
			<ul data-trigger="#signedInUserOptionsToggle" class="nav__links nav__links--products nav__links--mobile js-userAccount-Links js-nav-collapse-body offcanvasGroup1 collapse in hidden-md hidden-lg">
           	...
           	<ul class="nav__links nav__links--products js-offcanvas-links">
                <c:set var = "headerCol" value = "${fn:length(component.navigationNode.children)}"/>
                <c:if test="${headerCol <= 4}">
                   <c:set var = "col_header" value = "header4"/>
                </c:if>
                <c:if test="${headerCol == 5}">
                   <c:set var = "col_header" value = "header5"/>
                </c:if>
                <c:if test="${headerCol >= 6}">
                   <c:set var = "col_header" value = "col-md-2"/>
                </c:if>
                <div class="uppercase">${fn:escapeXml(string1)}</div>

				<c:forEach items="${component.navigationNode.children}" var="childLevel1"> 
					<li class="${col_header} auto nav__links--primary <c:if test="${not empty childLevel1.children}">nav__links--primary-has__sub js-enquire-has-sub</c:if>">
						...
						<c:choose>
							<c:when test="${!hasSubChild || (totalSubNavigationColumns > 0 && totalSubNavigationColumns <= 1)}">
								<c:set value="col-md-3" var="subNavigationClass"/>
				        ...
				        <c:set var = "headerCol2" value = "${fn:length(component.navigationNode.children)}"/>
                        <c:if test="${headerCol2 <= 4}">
                           <c:set var = "col_header2" value = "sub__navigation js_sub__navigation col-xs-3"/>
                        </c:if>
                        <c:if test="${headerCol2 == 5}">
                           <c:set var = "col_header2" value = "sub__navigation js_sub__navigation header5"/>
                        </c:if>
                        <c:if test="${headerCol2 >= 6}">
                           <c:set var = "col_header2" value = "sub__navigation col-xs-12"/>
                        </c:if>
