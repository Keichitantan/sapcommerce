 ...
 */
package mx.flexi.initialdata.setup;
...
import mx.flexi.initialdata.constants.FlexiB2CInitialDataConstants;
 ...
 */
@SystemSetup(extension = FlexiB2CInitialDataConstants.EXTENSIONNAME)
public class InitialDataSystemSetup extends AbstractSystemSetup
{
	 ...
	 */
	@SystemSetup(type = Type.ESSENTIAL, process = Process.ALL, extension = FlexiB2CInitialDataConstants.EXTENSIONNAME)
	 ...
	 *           the context provides the selected parameters and values
	 */
	@SystemSetup(type = Type.PROJECT, process = Process.ALL, extension = FlexiB2CInitialDataConstants.EXTENSIONNAME)
	public void createProjectData(final SystemSetupContext context)
	{
		final List<ImportData> importData = new ArrayList<ImportData>();

		final ImportData sampleImportData = new ImportData();
		sampleImportData.setProductCatalogName(FLEXI);
		sampleImportData.setContentCatalogNames(Arrays.asList(FLEXI));
		LOG.info("ProductCatalogName: " + sampleImportData.getProductCatalogName().toString());
		LOG.info("ContentCatalogName: " + sampleImportData.getContentCatalogNames().get(0).toString());
		sampleImportData.setStoreNames(Arrays.asList(FLEXI));
		LOG.info("StoreName: " + sampleImportData.getStoreNames().get(0).toString());
		importData.add(sampleImportData);

		LOG.info("Executing the FlexiB2Cinitialdata coredata impex...");
		getCoreDataImportService().execute(this, context, importData);
		getEventService().publishEvent(new CoreDataImportedEvent(context, importData));
